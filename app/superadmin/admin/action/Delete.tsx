...
function Delete({ adminId, reload, adminFoto }: { adminId: Number, reload: Function, adminFoto: String }) {
    ...
    const handleDelete = async (adminId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/superadmin/api/admin/${adminId}`)
        await supabase.storage
            .from(supabaseBUCKET)
            .remove([`foto-admin/${adminFoto}`]);
        reload()
        setIsLoading(false)
        Swal.fire({
            position: 'top-end',
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(adminId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
